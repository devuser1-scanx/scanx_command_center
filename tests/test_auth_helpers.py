...
from app.api.dependencies.auth import get_current_user, require_permission, require_role
from app.api.v1.routes.auth import get_client_ip, get_user_agent
...
def make_user() -> CCUser:
    ...
    admin_role = CCRole(id=1, code="admin", name="Admin", is_active=True)
    inactive_role = CCRole(id=2, code="disabled", name="Disabled", is_active=False)

    admin_role.permissions = [
        CCRolePermission(role=admin_role, permission=users_view),
        CCRolePermission(role=admin_role, permission=inactive_permission),
    ]
    inactive_role.permissions = [
        CCRolePermission(role=inactive_role, permission=users_view),
    ]
    user.roles = [
        CCUserRole(user=user, role=admin_role),
        CCUserRole(user=user, role=inactive_role),
    ]
    user.clinic_access = [
        CCUserClinicAccess(user=user, clinic_id=20, is_primary=False),
        CCUserClinicAccess(user=user, clinic_id=10, is_primary=True),
    ]

    return user


def make_request(headers: list[tuple[bytes, bytes]], host: str = "127.0.0.1") -> Request:
    return Request(
        {
            "type": "http",
            "method": "GET",
...
def test_collectors_ignore_inactive_roles_and_permissions() -> None:
    ...
    assert [access.clinic_id for access in collect_clinic_access(user)] == [10, 20]
...
def test_get_current_user_returns_active_access_token_user(monkeypatch: pytest.MonkeyPatch) -> None:
    user = make_user()

    def fake_get_user_by_id(db: FakeDb, user_id: int) -> CCUser | None:
        assert isinstance(db, FakeDb)
        assert user_id == user.id
        return user

    monkeypatch.setattr("app.api.dependencies.auth.get_user_by_id", fake_get_user_by_id)

    token = create_access_token(subject=str(user.id))

    assert get_current_user(token=token, db=FakeDb()) is user


def test_get_current_user_rejects_refresh_token() -> None:
    token = create_refresh_token(subject="1", session_id=1)

    with pytest.raises(HTTPException) as exc_info:
        get_current_user(token=token, db=FakeDb())

    assert exc_info.value.status_code == 401


def test_get_current_user_rejects_inactive_user(monkeypatch: pytest.MonkeyPatch) -> None:
    user = make_user()
    user.is_active = False
    monkeypatch.setattr("app.api.dependencies.auth.get_user_by_id", lambda db, user_id: user)
